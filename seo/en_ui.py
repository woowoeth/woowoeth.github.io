...
REGEX += [
    ...
    # 分类页（/en/t/<tag>/）的自述句。措辞照 geo_kit.hub_page 的英文分支抄，
    # 不另造一句 —— 同一句话两种写法，将来只会有一边被改到。
    # 分类页的门槛是 hub_min=3，所以复数形永远对。
    (re.compile(r"([^\s\"<>「」]+(?: [^\s\"<>「」]+)*) 里关于「([^」]+)」的 (\d+) 个条目。"),
     r"\3 entries on \1 tagged \2."),
]

# 分类页的眉标。放在 UI 里而不是 REGEX：它是个独立标签，
# 而「主题分类」（首页那一处）比它长，排序在前，不会被它吃掉。
# 分类页的眉标和几处「主题」标签。措辞照 geo_kit 里对应的英文分支抄。
# 打开 hubs=True 之后首页和 404 页各多出一行主题导航，就是这几条 ——
# 一个开关翻过来，界面层跟着多出几处要翻的字，靠 ⑪「产物里不许剩中文」
# 这道闸报出来，不是靠记得。
UI += [
    (">主题</p>", ">Topic</p>"),
...
# ── 量词那一类规则会造出双空格 ─────────────────────────────
# 中文里数字和量词之间本来就有一个空格（'26' + ' 个问题'），而这些规则的
# 替换串自己又带一个前导空格（'个问题' → ' questions'），拼出来是
# 「253  questions」「and 3  more」「80  situations · 253  questions」。
# 静态 HTML 里一个都看不到 —— 这几处是首页 JS 在运行时拼的，所以只有真的
# 打开页面才看得见。中文页上一个空格看不出来，英文页上两个空格很显眼。
#
# 修法不是把替换串的空格删掉：有些地方数字和量词是**贴着**的
# （'${items.length}位'），那里反而需要补空格。所以给每条这样的规则再加
# 一条「键前面带空格」的版本 —— 它更长，排序时排在前面，有空格的那种
# 先被它吃掉；没空格的那种仍然走原来那条。
#
