...
CHECKS = [
    ...
    # 章节页三语同 slug，套前缀就对；话题页不是（slug 按各自语言的标签名派生）。
    # 上一条抓「指向 404」，抓不住「少写一条 en」—— 少写不报错，只是悄悄没了。
    ("话题互指", "check_lang_pairs.py", "三语话题页按 seo/topic_pairs.py 互相指到"),
    # 描述里的数字必须等于真实数量 —— 写死的数字一定会过期（原来是「75位」，实际 171）。
    ("PWA", "check_pwa.py", "三语装得上 / 离线打得开 / 描述数字不过期"),
    # 和 PWA 一样属于「网站之外的分发层」：改坏了页面照样全绿。所以这道闸真起一个
    # MCP server 问它三句话，而不是查文件在不在 —— 文件在、协议坏，是最常见的那种坏法。
    ("MCP / Skill", "check_tools.py", "server 真应答且取得到章节 / Skill 的硬边界还在"),
