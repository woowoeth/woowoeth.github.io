...
/* 列宽定成 330px，让列数自己算出来——这样卡片在任何屏幕上宽度都接近，
   行长一致。实测：375px→1 列（卡宽 327，一屏 4.2 张）、768px→2 列（354）、
   1280px→3 列（349）。原先是 columns:150px + @900px 的 230px，
   375px 上两列、卡宽 158，中文两三个字就换行，卡片被撑到 263px 高，一屏 5.9 张。
   改成单列后卡片反而矮到 178px——变宽让行数减半。 */
#hwx .feed{columns:330px;column-gap:12px;margin-top:14px}
#hwx .feed>*{break-inside:avoid;width:100%;margin:0 0 16px}
#hwx .nc-feed{columns:330px;column-gap:12px;margin-top:14px}
...
/* 900px 那条固定列宽已删：列宽由上面的 330px 统一决定，不再按断点切。 */
