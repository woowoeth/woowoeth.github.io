...
:root[data-theme="dark"] #hwx .qc .who,:root[data-theme="dark"] #hwx .pc .it,:root[data-theme="dark"] #hwx .pc .tag,:root[data-theme="dark"] #hwx .pc .ls,:root[data-theme="dark"] #hwx .nc .w,:root[data-theme="dark"] #hwx .nc .wh,:root[data-theme="dark"] #hwx .nc .pn{color:#a79e8b!important}
...
/* 移动端保持双列。试过单列（列宽 330px）：一屏 5.9→4.2 张、卡片因为变宽反而
   从 263px 矮到 178px，读起来也更顺——但产品负责人不喜欢单排的样子，改回双列。
   代价是一屏仍然 6 张左右，这一点是清楚的、被接受的取舍，不是没注意到。 */
#hwx .feed{columns:150px;column-gap:12px;margin-top:14px}
#hwx .feed>*{break-inside:avoid;width:100%;margin:0 0 16px}
#hwx .nc-feed{columns:150px;column-gap:12px;margin-top:14px}
#hwx .nc-feed>*{break-inside:avoid;width:100%;margin:0 0 16px}
...
/* 「什么时候翻开它」：sc（章节 apply 的「局面」句）早就在 NC 数据里，一直没渲染。
   金句卡有「用在……的时候」、今日一篇有，唯独人物/章节卡没有——而这恰恰是
   知识库卡片最该给的：不是这篇讲什么，是你什么时候用得上。
   顺带把双列一屏的张数从 5.8 压下来，靠的是加有用的内容，不是加留白。 */
#hwx .nc .wh{font-size:12.5px;color:var(--muted);line-height:1.7;margin-top:10px;border-top:1px dashed var(--line);padding-top:9px}
...
D.NC.forEach(function(e,i){
  ncCells.push('<a class="nc" href="'+e.u+'" data-h="'+esc(e.pn+' · '+e.cn)+'" style="--tint:'+(D.CT[e.c]||'transparent')+';--tintd:'+(D.CTD[e.c]||'transparent')+'"><span class="pn"><i>'+e.pn+'</i><span class="nb">新</span></span><b>'+e.cn+'</b><span class="w">'+e.w+'</span>'+(e.q?'<span class="q">'+e.q+'</span>':'')+(e.sc?'<span class="wh">什么时候翻开它：'+e.sc+'</span>':'')+'</a>');
  if(i%4===3){
    var pe=null;
