...
/* 这里**不要**再补一条 `.layout{grid-template-columns:minmax(0,1fr)}`。
   防横向撑开的那条住在 assets/hw-chapter.css，三种语言共用；在这里再写一份
   权重更高的，等于给英文页加了第二层保护 —— 听起来更稳，实际后果是
   `gate_selftest` 里「窄屏·横向撑开」那条反向注入**再也撑不开页面**，
   变成一条永远绿的死闸。多一层冗余保护换掉一条能报警的判据，不划算。 */
...
/* ── Playfair 只做大标题 ─────────────────────────────────
   ...
   规矩按**角色**分，不按字号分：一张卡的标题归 Playfair，卡上的辅助文字
   归 Source Serif 4，界面文字（标签、胶囊、按钮）归无衬线。

   —— 上一版这条规矩写的是「≥20px 的标题归 Playfair」，是错的，而且错在
   一个会反复咬人的地方：**拿字号当判据，等于拿汉字的排版当判据**。这几个
   字号（16.5 / 19 / 21px）全是照汉字定的，同一个位置汉字 16.5px 就够，
   英文不够。于是「16.5px 的金句卡标题」被判成正文、「19px 的新章卡标题」
   被判成标题 —— 两张卡的同一个角色分到了两种字体。中文那边四种卡的标题
   是同一套宋体，一个声音；英文这边裂成两半，扫读时能感觉到不齐，但说不
   出哪不齐。判据必须落在角色上，字号是角色的**结果**，不是它的依据。 */
...
/* 金句卡的引文、问题卡的问句 —— 这两个是各自那张卡的标题（卡上最大的
   一行，也是扫读时唯一会看的一行），跟人物卡的名字、新章卡的标题同级。
   四种卡的标题统一到同一个字号（21px）—— 中文那边是两档（名字/章题 19px、
   引文/问句 16.5px），英文这里并成一档：汉字 16.5px 撑得住一行标题，
   Playfair 到不了，16.5px 上它的高对比笔画和发丝衬线都糊掉，读出来只是
   一个普通粗衬线。
   行高和字距也要跟着改：汉字要 1.65~1.75 的行高、正的字距才透气，
   同样的值给拉丁字母就是散的。 */
html[lang="en"] #hwx .qc .v,
html[lang="en"] #hwx .kc .t,
html[lang="en"] #hwx .nc b,
html[lang="en"] #hwx .pc b{font-family:var(--display);font-size:21px;
  line-height:1.34;letter-spacing:-.012em}
/* 问题卡下面那个「谁问过这事」的署名：它是卡上的第二层，比标题小一档，
   但仍然是「这张卡讲谁」，所以也走展示体。原来是 Source Serif 4 13.5px，
   跟同一张卡的标题各说各的。 */
